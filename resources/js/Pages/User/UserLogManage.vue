<template>
    ...
    <MainLayout title="User Log">
        <div class="flex flex-col">
            <DxDataGrid ref="datagridRef" :data-source="logData" :remote-operations="false">
                <DxSelection mode="none" />
                <!-- Your columns here -->
                <DxColumn data-field="status_code" caption="Code" width="70" alignment="center" cell-template="status-code" />
                <template #status-code="{ data }">
                    <span v-if="data.data.status_code >= 200 && data.data.status_code < 300" class="px-2 py-1 rounded-md bg-primary text-white text-[10px]">{{ data.data.status_code }}</span>
                    <span v-else-if="data.data.status_code >= 300 && data.data.status_code < 400" class="px-2 py-1 rounded-md bg-secondary text-white text-[10px]">{{ data.data.status_code }}</span>
                    <span v-else class="px-2 py-1 rounded-md bg-danger text-white text-[10px]">{{ data.data.status_code }}</span>
                </template>
                <DxColumn data-field="method" caption="Method" width="100" alignment="center" />
                <DxColumn data-field="path" caption="Path" :minWidth="200" />
                <DxColumn data-field="timestamp" caption="Timestamp" data-type="datetime" format="dd/MM/yyyy HH:mm" width="160" />
                    ...
                    <div class="bg-white w-full mx-auto shadow-md rounded-lg overflow-hidden mt-2" v-if="logData.data.request_body != null && logData.data.request_body != undefined && Object.keys(logData.data.request_body).length > 0">
                        <div class="px-6 py-4">
                            ...
                            <div class="bg-gray-200 p-2 text-sm">
                                <pre class="whitespace-pre-wrap">{{ JSON.stringify(logData.data.request_body, null, 2) }}</pre>
                            </div>
                        </div>
                    </div>
